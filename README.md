...
- **Credibility scoring**: a "Verity Score" is computed per station and per topic — weighted so a false claim on a high-importance story hurts the score far more than an error on a trivial one, and scaled by the fact-checker's own confidence.
